        ...
        // --- CONTENT GENERATION ---
         ...
         if (analysisType === 'cronbach' || analysisType === 'omega') {
            ...
            if (results.sem && results.sem.path_coefficients) {
                ...
                doc.setFontSize(14);
                doc.setFont('NotoSans', 'bold');
                doc.text('Mô hình nghiên cứu (Research Model)', 15, yPos);
                yPos += 10;
                ...
                if (paths.length > 0) {
                    doc.text('Các giả thuyết đường dẫn:', 15, yPos);
                    yPos += 6;
                    paths.forEach(p => {
                        doc.text(`• ${p}`, 20, yPos);
                        yPos += 6;
            ...
            if (results.cronbach) {
                ...
                doc.setFontSize(12);
                doc.setFont('NotoSans', 'bold');
                doc.text('1.1 Bảng Tổng hợp Độ tin cậy các Thang đo', 15, yPos);
                yPos += 8;
                ...
                doc.setFontSize(12);
                doc.setFont('NotoSans', 'bold');
                doc.text('1.2 Phân tích Chi tiết', 15, yPos);
                yPos += 8;
            ...
            if (results.efa && results.efa.data) {
                ...
                doc.setFontSize(10);
                doc.setFont('NotoSans', 'normal');
                if (efaRes.kmo !== undefined) doc.text(`KMO Measure of Sampling Adequacy: ${Number(efaRes.kmo).toFixed(3)}`, 15, yPos);
                yPos += 7;
                if (efaRes.bartlettP !== undefined) doc.text(`Bartlett's Test of Sphericity - p-value: ${Number(efaRes.bartlettP).toFixed(4)}`, 15, yPos);
                yPos += 10;
            ...
            if (results.sem) {
                ...
                if (semRes.validity && semRes.validity.cronbach) {
                    ...
                    doc.setFontSize(12);
                    doc.setFont('NotoSans', 'bold');
                    doc.text('3.1 Độ tin cậy cấu trúc & Tính hội tụ (Construct Reliability and Validity)', 15, yPos);
                    yPos += 8;
                ...
                if (semRes.htmt && Object.keys(semRes.htmt).length > 0) {
                    ...
                    doc.setFontSize(12);
                    doc.setFont('NotoSans', 'bold');
                    doc.text('3.2 Tính phân biệt (Discriminant Validity - HTMT)', 15, yPos);
                    yPos += 8;
                ...
                if (semRes.vif && semRes.vif.vif_values) {
                    ...
                    doc.setFontSize(12);
                    doc.setFont('NotoSans', 'bold');
                    doc.text('3.3 Đánh giá Đa cộng tuyến (VIF)', 15, yPos);
                    yPos += 8;
                ...
                if (semRes.bootstrapping && semRes.bootstrapping.boot_paths) {
                    ...
                    doc.setFontSize(12);
                    doc.setFont('NotoSans', 'bold');
                    doc.text('3.4 Kiểm định giả thuyết (Path Significance - Bootstrapping)', 15, yPos);
                    yPos += 8;
                    ...
                    doc.setFontSize(12);
                    doc.setFont('NotoSans', 'bold');
                    doc.text('3.4 Hệ số tác động (Path Coefficients)', 15, yPos);
                    yPos += 8;
