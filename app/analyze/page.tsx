...
import { useAnalyzeLifecycle } from './hooks/useAnalyzeLifecycle';
import { useAnalysisRunner } from './hooks/useAnalysisRunner';
import { AnalyzeStepRenderer } from './components/AnalyzeStepRenderer';
...
function AnalyzeContent() {
    ...
    const {
        ...
        setNcsBalance, setToast: showToast, setShowDemographics
    });
    ...
    const { isAnalyzing, setIsAnalyzing, analysisProgress, runAnalysis } = useAnalysisRunner({
        data, getNumericColumns, user, setStep, setAnalysisType, setRequiredCredits, setCurrentAnalysisCost,
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
                ...
                <div className="container mx-auto px-2 md:px-6 py-4 md:py-8 overflow-x-auto no-scrollbar">
                    ...
                    <div className="py-8">
                        <AnalyzeStepRenderer
                            ...
                            handleDataLoaded={(data, fname) => {
                                setData(data);
                                setFilename(fname);
                                setStep('profile' as any);
                            }}
            ...
            />
            {showDemographics && (
                <DemographicSurvey isOpen={showDemographics} onComplete={() => setShowDemographics(false)} />
            )}
            {showApplicability && (
                <ApplicabilitySurvey isOpen={showApplicability} onComplete={() => setShowApplicability(false)} onCancel={() => setShowApplicability(false)} />
            )}
